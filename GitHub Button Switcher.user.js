...
(function () {
      ...
      XMLHttpRequest.prototype.send = function () {
        try {
          ...
          if (xhr.upload) {
            ...
            // ждём именно ответ сервера, а не только момент, когда байты ушли,
            // иначе кнопка оживает раньше, чем файл принят
  ...
  function slimUpload() {
    ...
    const isReady = () => {
      if (up.inflight > 0) return false;                                  // прямо сейчас идёт отправка
      if (up.done > 0) return true;                                       // файл принят сервером
