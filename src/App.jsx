...
function Avatar({ src, owner, repo, branch, iconUrl, readmeIcon, resolved, onResolved }) {
  const br = branch || "HEAD";
  const raw = (p) => `https://raw.githubusercontent.com/${owner}/${repo}/${br}/${p}`;
  const cands = [];
  if (resolved) cands.push(resolved); // уже найденная ранее — без повторного перебора
  cands.push(
    raw("fastlane/metadata/android/en-US/images/icon.png"),
    raw("metadata/en-US/images/icon.png"),
    raw("resources/icon.png"),
    raw("app/src/main/ic_launcher-playstore.png"),
    raw("app/src/main/res/mipmap-xxxhdpi/ic_launcher.png"),
    raw("app/src/main/res/mipmap-xxhdpi/ic_launcher.png"),
    raw("app/src/main/res/mipmap-xxxhdpi/ic_launcher_round.png"),
    raw("app/src/main/res/mipmap-xhdpi/ic_launcher.png"),
    raw("app/src/main/res/drawable/ic_launcher.png"),
    raw("assets/icon.png"),
    raw("icon.png"),
    raw("logo.png"),
  );
  // запасные варианты, только если иконка не найдена по путям
  if (iconUrl) cands.push(iconUrl);          // лого организации
  if (readmeIcon) cands.push(readmeIcon);    // картинка из README
  ...
  if (i < cands.length)
    return (
      <img
        ...
        alt=""
        loading="lazy"
        onLoad={() => { if (onResolved && cands[i] !== resolved) onResolved(cands[i]); }}
        onError={() => setI(i + 1)}
...
export default function App() {
  ...
  const checkOne = useCallback(async (s) => {
    try {
      ...
      let readmeIcon = s.readmeIcon || null;
      // иконку-мету (ветка / лого-орг / README) резолвим ОДИН раз и кэшируем
      if (!s.iconResolved) {
        try {
          ...
          if (ri.ok) {
            ...
            iconUrl = info.owner && info.owner.type === "Organization" ? info.owner.avatar_url : null;
          }
        } catch {}
        try {
          ...
          if (rr.ok) {
            ...
            const base = (rj.download_url || `https://raw.githubusercontent.com/${s.owner}/${s.repo}/${branch}/README.md`)
              .replace(/\/[^/]*$/, "/");
            const m = md.match(/!\[[^\]]*\]\(\s*<?([^)\s>]+)/) || md.match(/<img[^>]+src=["']([^"']+)["']/i);
            if (m && m[1]) {
              let u = m[1].trim();
              if (/^https?:\/\//i.test(u)) readmeIcon = u;
              else readmeIcon = base + u.replace(/^\.?\//, "");
              readmeIcon = readmeIcon.replace("github.com", "raw.githubusercontent.com").replace("/blob/", "/");
            }
          }
        } catch {}
      }
      const meta = { iconUrl, branch, readmeIcon, iconResolved: true };
        ...
        {sources.map((s) => (
          <div key={s.id} style={S.row}
            ...
            <Avatar
              ...
              iconUrl={s.iconUrl} readmeIcon={s.readmeIcon} resolved={s.resolvedIcon}
              onResolved={(url) => {
                const cur = store.read();
