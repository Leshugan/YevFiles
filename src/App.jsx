...
export default function App() {
  ...
  const goUp = () => {
    ...
    if (path) { setSlide(-1); setTimeout(() => setSlide(0), 300); setTabPath(parent(path)); }
  };
  ...
  const openArchive = async (e) => {
    ...
    try {
      ...
      if (r.encrypted && !getCachedPw(e.uri)) {
        // спросить и проверить пароль до показа содержимого
        const test = entries.filter((x) => x.size > 0).sort((a, b) => a.size - b.size)[0] || entries[0];
        const destDir = await absPath("Download/.yevtmp");
        let ok = false;
        for (let t = 0; t < 3 && !ok; t++) {
          const pw = await askPassword(t > 0);
          if (pw == null || pw === "") { setArcView(null); return; }
          try { await Apps.zipExtract({ uri: e.uri, entry: test.name, dest: destDir + "/.pwtest", password: pw }); setCachedPw(e.uri, pw); ok = true; }
    ...
    <div style={{ ...S.app, ...(THEMES[theme] || THEMES.dark), fontFamily: fontCss }}>
      ...
      <div style={S.crumb}>
        <span style={{ flex: 1, display: "flex", alignItems: "center", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {cur && cur.src ? <span onClick={goUp} style={{ display: "inline-flex", alignItems: "center", gap: 6, color: ACC }}><Svg d={I.back} size={18} /> {cur.src.stack.map((x) => x.name).join(" / ")}</span>
            ...
            <div style={{ overflow: "auto", touchAction: dDragId ? "none" : "auto" }}>
              {(() => {
                const avail = [
                  ...
                  { id: "storage", kind: "st", n: "Storage", sub: "/storage/emulated/0", d: I.folder, c: "#4FC3D9", p: "" },
                ];
