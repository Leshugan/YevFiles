...
/* ===== Files — leshugan.fm =====
   ...
   Вкладки со свайпом, нижняя панель + тулбар выделения. */
...
/* ---- хранилище вкладок (не критично, с фолбэком) ---- */
let mem = null;
const loadTabs = () => {
  try { return JSON.parse(localStorage.getItem(TKEY)); } catch { return mem; }
};
const saveTabs = (t) => {
  mem = t;
...
const I = {
  ...
  file: <><path d="M6 2h9l5 5v15a0 0 0 0 1 0 0H6a0 0 0 0 1 0 0z" /><path d="M14 2v6h6" /></>,
};
const Svg = ({ d, size = 24, fill = "none" }) => (
...
export default function App() {
  const [tabs, setTabs] = useState(() =>
    loadTabs() || [{ id: 1, name: "Storage", path: "" }]);
  ...
  const [clip, setClip] = useState(null); // {mode:'cut'|'copy', dir, items:[]}
  const [query, setQuery] = useState(null); // null = поиск выключен
  const [createOpen, setCreateOpen] = useState(false);
  const [slide, setSlide] = useState(0); // анимация смены вкладки
  ...
  const list = useCallback(async () => {
    ...
    try {
      ...
      setError(e.message || "Нет доступа к хранилищу");
      setEntries([]);
    }
    setLoading(false);
  }, [path]);
  ...
  useEffect(() => { list(); exitSel(); /* eslint-disable-next-line */ }, [active, path]);

  /* ---- выделение ---- */
  const exitSel = () => { setSel(new Set()); setSelMode(false); };
  const toggle = (name) => {
    const n = new Set(sel);
  ...
  const selectAll = () => {
    setSelMode(true);
    setSel(new Set(visible.map((e) => e.name)));
  };
  ...
  const open = (e) => {
    ...
    else { setSelMode(true); setSel(new Set([e.name])); }
  };
  const longPress = (e) => { setSelMode(true); toggle(e.name); };
  const goUp = () => { if (path) setTabPath(parent(path)); };
  const setTabPath = (p) => {
    const t = tabs.map((x, i) => (i === active ? { ...x, path: p } : x));
    persist(t);
  };
  ...
  const addTab = () => {
    const id = Date.now();
    const t = [...tabs, { id, name: "Storage", path: "" }];
  ...
  const doCreateTxt = async () => {
    setCreateOpen(false);
    let name = prompt("Имя файла (.txt)");
    if (!name) return;
    if (!/\.txt$/i.test(name)) name += ".txt";
    try {
      await Filesystem.writeFile({ path: rel(name), directory: DIR, data: "", encoding: Encoding.UTF8 });
      refresh();
    } catch (e) { alert("Ошибка: " + e.message); }
  };

  const doDelete = async () => {
    if (!confirm(`Удалить (${sel.size})?`)) return;
    for (const name of sel) {
      const e = entries.find((x) => x.name === name);
      try {
        if (e?.type === "directory")
          await Filesystem.rmdir({ path: rel(name), directory: DIR, recursive: true });
  ...
  const doRename = async () => {
    ...
    try {
      await Filesystem.rename({ from: rel(name), to: rel(nn), directory: DIR, toDirectory: DIR });
      exitSel(); refresh();
    } catch (e) { alert("Ошибка: " + e.message); }
  };

  const grab = (mode) => {
    setClip({ mode, dir: path, items: [...sel] });
    exitSel();
  };
  const paste = async () => {
    if (!clip) return;
    for (const name of clip.items) {
      const from = join(clip.dir, name);
      const to = rel(name);
      if (from === to) continue;
      try {
        if (clip.mode === "copy")
          await Filesystem.copy({ from, to, directory: DIR, toDirectory: DIR });
        else
  ...
  /* ---- видимый список (с учётом поиска) ---- */
  const visible = query
    ? entries.filter((e) => e.name.toLowerCase().includes(query.toLowerCase()))
    : entries;

  /* long-press таймер */
  const lpTimer = useRef(); const lpFired = useRef(false);
  const rowDown = (e) => { lpFired.current = false; lpTimer.current = setTimeout(() => { lpFired.current = true; longPress(e); }, 450); };
  const rowUp = () => clearTimeout(lpTimer.current);
  const rowClick = (e) => { if (lpFired.current) { lpFired.current = false; return; } open(e); };
    ...
    <div style={S.app}>
      ...
      <main style={S.list} onTouchStart={onTS} onTouchMove={onTM}>
        <div key={active} style={{
          ...S.slideWrap,
          animation: slide ? `fm-in-${slide > 0 ? "r" : "l"} .22s ease` : "none",
        }}>
          {loading && <div style={S.note}>Загрузка…</div>}
          {error && <div style={{ ...S.note, color: "#e06b6b" }}>{error}<br />
          ...
          {visible.map((e) => {
            ...
            return (
              <div key={e.name} style={{ ...S.row, ...(isSel ? S.rowSel : {}) }}
                onClick={() => rowClick(e)}
                onTouchStart={() => rowDown(e)} onTouchEnd={rowUp} onTouchMove={rowUp}
                onContextMenu={(ev) => { ev.preventDefault(); longPress(e); }}>
                ...
                {selMode && (
                  <span style={{ ...S.check, ...(isSel ? S.checkOn : {}) }}>{isSel ? "✓" : ""}</span>
                )}
              </div>
            );
          })}
      ...
      {selMode ? (
        <nav style={S.bottom}>
          ...
          <Btn onClick={doDelete} icon={I.trash} label="Удалить" />
          <Btn onClick={doRename} text="A" label="Имя" disabled={sel.size !== 1} underline />
        </nav>
      ) : (
        <nav style={S.bottom}>
          <Btn onClick={() => setQuery(query === null ? "" : null)} icon={I.search} label="Поиск" />
          <Btn onClick={selectAll} icon={I.selectAll} label="Все" />
          {clip
...
function Btn({ onClick, icon, text, label, accent, underline, disabled }) {
  return (
    <button onClick={disabled ? undefined : onClick}
      style={{ ...S.btn, color: disabled ? "#445" : accent ? ACC : TXT }}>
...
const S = {
  app: { display: "flex", flexDirection: "column", height: "100vh", background: BG,
    color: TXT, fontFamily: "system-ui,-apple-system,Roboto,sans-serif",
    userSelect: "none", overflow: "hidden" },
  tabsbar: { display: "flex", alignItems: "center", background: BAR,
    borderBottom: "1px solid #0a0f15", flexShrink: 0 },
  tabs: { display: "flex", overflowX: "auto", flex: 1 },
  tab: { display: "flex", alignItems: "center", gap: 6, padding: "12px 14px",
    fontSize: 14, color: SUB, whiteSpace: "nowrap", borderBottom: "2px solid transparent" },
  tabActive: { color: TXT, borderBottom: `2px solid ${ACC}` },
  tabX: { fontSize: 17, color: SUB, padding: "0 2px" },
  tabAdd: { border: "none", background: "transparent", color: ACC, fontSize: 22,
  ...
  searchInput: { flex: 1, padding: "10px 12px", borderRadius: 8, border: "1px solid #2a3a47",
    background: BAR, color: TXT, fontSize: 15, outline: "none", userSelect: "text" },
  ...
  createMenu: { position: "absolute", bottom: 60, right: 0, zIndex: 9, background: ROW2,
    borderRadius: 10, overflow: "hidden", border: "1px solid #2a3a47",
    boxShadow: "0 6px 24px #0009", minWidth: 130 },
